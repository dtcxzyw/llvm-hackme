...
- **Fuzz parallelism fixed at 1** — `LLVM_HACKME_MAX_FUZZ_PARALLELISM` defaults to 1 and must not be increased. The fuzzer runs mutation iterations sequentially; code paths that only trigger with parallelism > 1 are dead code and do not need fixes.
