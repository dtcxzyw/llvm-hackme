...
export default tool({
  description:
    "Run the PR (patched) opt on LLVM IR text.  Returns exit code, signal, crashed, stdout, and stderr.  -S is always passed; do NOT add -S or -o flags to opt_args.",
  args: {
    ir: tool.schema
      .string()
  ...
  async execute(args) {
    ...
    cmd.push(ctx.pr_opt, "-S", tmp, ...extra)
