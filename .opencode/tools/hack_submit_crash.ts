...
export default tool({
  description:
    "Submit a crash reproducer to the Python service for verification.  If the crash is confirmed, the process exits.  Otherwise returns the verification failure reason so you can retry.",
  args: {
    ir: tool.schema
      .string()
  ...
  async execute(args) {
    ...
    try {
      ...
      if (resp.success) {
        return "Crash confirmed and reported.  Exiting."
      }
